...
export class Sensor_THREE extends Transducer_THREE {
  ...
  set minIntensitySensitivity(value) {
    if (typeof value !== 'number' || value <= 0 || !isFinite(value) || isNaN(value)) {
      throw new Error('minIntensitySensitivity must be a positive number and finite.');
    }
    this.#minIntensitySensitivity = value;
    this.#cachedSensitivityDenominator = Sensor_THREE.FOUR_PI * this.#minIntensitySensitivity;

    //update the effective range helper
    this.fieldOfEffect_HelperRange = this.calculateDetectionRange(this.proxyPower);
  }
  ...
  isTargetDetectable(targetObject) {
    ...
    if (typeof targetObject.power !== 'number' || targetObject.power <= 0) {
      ...
      throw new Error('Target object must have a positive power property. ' + typeof targetObject);
    }
    ...
    if (angle > this.fieldOfEffect_AngleFull / 2) {
      ...
      const receivedIntensity = this.calculateReceivedIntensity(targetObject.power, distance);

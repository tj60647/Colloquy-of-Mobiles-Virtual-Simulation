...
export class Transform {
    ...
    set x(v: number) { this._position.x = v; this._needsPositionUpdate = true; this.markChildrenForUpdate(); }
    get y() { return this._position.y; }
    set y(v: number) { this._position.y = v; this._needsPositionUpdate = true; this.markChildrenForUpdate(); }
    get z() { return this._position.z; }
    set z(v: number) { this._position.z = v; this._needsPositionUpdate = true; this.markChildrenForUpdate(); }

    get localPosition(): Vector3 { return this._position.clone(); }
    ...
    get yaw() { return this._rotation.yaw; }
    set yaw(v: number) { this._rotation.yaw = v; this._needsOrientationUpdate = true; this.markChildrenForUpdate(); }
    get pitch() { return this._rotation.pitch; }
    set pitch(v: number) { this._rotation.pitch = v; this._needsOrientationUpdate = true; this.markChildrenForUpdate(); }
    get roll() { return this._rotation.roll; }
    set roll(v: number) { this._rotation.roll = v; this._needsOrientationUpdate = true; this.markChildrenForUpdate(); }
    ...
    private calculateOrientation(): Euler {
        const parentOri = this.parent ? this.parent.getGlobalOrientation() : { yaw: 0, pitch: 0, roll: 0 };
        return {
    ...
    private applyRotation(v: Vector3, yaw: number, pitch: number, roll: number): Vector3 {
        ...
        let cr = Math.cos(radRoll), sr = Math.sin(radRoll);
        let x1 = x * cr - y * sr;
        let y1 = x * sr + y * cr;
        let z1 = z;
        ...
        let cp = Math.cos(radPitch), sp = Math.sin(radPitch);
        let x2 = x1;
        let y2 = y1 * cp - z1 * sp;
        let z2 = y1 * sp + z1 * cp;
        ...
        let cy = Math.cos(radYaw), sy = Math.sin(radYaw);
        let x3 = x2 * cy + z2 * sy;
        let y3 = y2;
        let z3 = -x2 * sy + z2 * cy;
    ...
    private parentHasChanged(): boolean {
        ...
        // Check Position
        if (Math.abs(currentPos.x - this._lastParentGlobalPosition.x) > 0.0001 ||
            Math.abs(currentPos.y - this._lastParentGlobalPosition.y) > 0.0001 ||
            Math.abs(currentPos.z - this._lastParentGlobalPosition.z) > 0.0001) return true;

        // Check Orientation
        if (Math.abs(currentOri.yaw - this._lastParentGlobalOrientation.yaw) > 0.0001 ||
            Math.abs(currentOri.pitch - this._lastParentGlobalOrientation.pitch) > 0.0001 ||
            Math.abs(currentOri.roll - this._lastParentGlobalOrientation.roll) > 0.0001) return true;

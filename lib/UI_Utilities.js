 ...
 */
export function formatValue(value, decimalPlaces = 3, padWidth = 1, leadingZeroes = false) {
  ...
  return `${value < 0 ? '-' : '+'}${integerPart}${decimalPart ? `.${decimalPart}` : ''}`;
}

/**
 * Generates an rgba color string from a hex color value with optional alpha and intensity adjustments.
 * The intensity parameter can be used to darken or lighten the color.
 * The alpha parameter controls the transparency of the color.
 * The hex color value is expected to be in the format 0xRRGGBB.
 * @param {string} hex - The hex color value in the format #RRGGBB.
 * @param {number} [alpha=1.0] - The alpha value (opacity) for the rgba color. Defaults to 1.0.
 * @param {number} [intensity=1.0] - The intensity value for darkening or lightening the color. Defaults to 1.0.

...
export class Actuator_THREE extends Transducer_THREE {
  ...
  calculateEffectiveRange(minIntensitySensitivity) {
    if (minIntensitySensitivity <= 0) {
      throw new Error('Minimum intensity sensitivity must be greater than zero.');
    }
    return Math.sqrt(this.#power / (minIntensitySensitivity * Transducer_THREE.FOUR_PI));
  }
  ...
  isTargetInFieldOfEffect(targetObject) {
    ...
    if (
      ...
      throw new Error('Target object must have a positive minIntensitySensitivity property.');
    }

...
export class OscillatorSystem_THREE extends THREE.Object3D {
  ...
  constructor(
    ...
    if (frameRate <= 0) {
      throw new Error(
        "OscillatorSystem_THREE.constructor: 'frameRate' must be a positive value."
      );
    }
    if (maxVelocity <= 0) {
      throw new Error(
  ...
  set minPosition(value) {
    if (value >= this.maxPosition) {
      throw new Error(
        "Oscillator_THREE: minPosition must be less than maxPosition."
      );
    }
  ...
  set maxPosition(value) {
    if (value <= this.minPosition) {
      throw new Error(
        "Oscillator_THREE: maxPosition must be greater than minPosition."
      );
    }
  ...
  set tolerance(value) {
    if (value < 0) {
      throw new Error(
        "Oscillator_THREE: tolerance must be a non-negative value."
      );
    }
  ...
  set maxVelocity(value) {
    if (value <= 0) {
      throw new Error(
        "Oscillator_THREE: maxVelocity must be a positive value."
      );
    }
  ...
  set maxAcceleration(value) {
    if (value <= 0) {
      throw new Error(
        "Oscillator_THREE: maxAcceleration must be a positive value."
      );
    }
  ...
  set maxJerk(value) {
    if (value < 0) {
      throw new Error(
        "Oscillator_THREE: maxJerk must be a non-negative value."
      );
    }
    this.#maxJerk = value;
  }
  ...
  setMotionRequest(motionRequest) {
    ...
    if (
      !Object.values(OscillatorSystem_THREE.MOTION_REQUEST).includes(
        motionRequest
      )
    ) {
      console.warn(
      ...
      this.#currentMotionRequest =
        OscillatorSystem_THREE.MOTION_REQUEST.UNKNOWN;
      return;
    }
    ...
    if (motionRequest === OscillatorSystem_THREE.MOTION_REQUEST.STOP) {
      ...
      motionRequest === OscillatorSystem_THREE.MOTION_REQUEST.RELEASE
    ) {
  ...
  computeMotionProfile() {
    ...
    if (!this.#motionProfile.profile.length) {
      console.log(
        `Oscillator_THREE.computeMotionProfile: No motion profile generated.`
      );
    }
  ...
  handleStopRequest() {
    if (
      ...
      if (!this.#stopTargetReached) {
        ...
        console.log(
          `OscillatorSystem_THREE.handleStopRequest: Reached reinforcement position.`
        );
      }
  ...
  #createRangeOfMotionHelper() {
    ...
    const pointsOriginToMin = [
      ...
      new THREE.Vector3(
        size * Math.sin(this.#minPosition),
        0,
        size * Math.cos(this.#minPosition)
      ), // Min position in XZ plane
    ];
    const geometryXZ_min = new THREE.BufferGeometry().setFromPoints(
      pointsOriginToMin
    );
    ...
    const pointsOriginToMax = [
      new THREE.Vector3(0, 0, 0), // Origin
      new THREE.Vector3(
        size * Math.sin(this.#maxPosition),
        0,
        size * Math.cos(this.#maxPosition)
      ), // Max position in XZ plane
    ];
    const geometryXZ_max = new THREE.BufferGeometry().setFromPoints(
      pointsOriginToMax
    );
  ...
  #createOscillator() {
    ...
    if (this.#oscillator) {
      ...
      if (this.#oscillator.children.length > 0) {
        ...
        throw new Error(
          "OscillatorSystem_THREE.#createOscillator: Oscillator has children."
        );
      } else {

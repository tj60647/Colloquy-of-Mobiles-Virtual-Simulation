...
export class Environment {
  ...
  toJSON() {
    return {
      ...
      interactions: [], // TODO: Track interactions when implemented
      timestamp: Date.now(),
    };
  }
  ...
  static fromJSON(json: any): Environment {
    ...
    for (const mobileJson of json.mobiles) {
      if (mobileJson.parentId && mobileJson.parentId !== mobileJson.id) {
        debugLog.push(
          `Looking up ${mobileJson.name} (id=${mobileJson.id}) and parent (id=${mobileJson.parentId})`
        );
    ...
    // Debug log (browser-compatible)
    if (debugLog.length > 0) {
      console.log('[Environment.fromJSON] Debug log:\n' + debugLog.join('\n'));
    }

//import classes
import { DriveManager, DriveState } from "./DriveManager.js";
...
export class DriveVisualization {
  ...
  createStaticImage() {
    ...
    pg.fill(
      lerpColor(
        this.hexToP5Color(
          DriveVisualization.getDriveStateColor("satisfied_and_indifferent")
        ),
        color(255),
        0.7
      )
    ...
    pg.fill(
      lerpColor(
        this.hexToP5Color(
          DriveVisualization.getDriveStateColor("O_satisfaction_search")
        ),
        color(255),
        0.7
      )
    ...
    pg.fill(
      lerpColor(
        this.hexToP5Color(
          DriveVisualization.getDriveStateColor("P_satisfaction_search")
        ),
        color(255),
        0.7
      )
    ...
    pg.fill(
      lerpColor(
        this.hexToP5Color(
          DriveVisualization.getDriveStateColor(
            "either_O_or_P_satisfaction_search"
          )
        ),
        color(255),
        0.7
    ...
    pg.stroke(
      this.hexToP5Color(DriveVisualization.getDriveStateColor("grid_line"))
    );
  ...
  renderDriveHistory() {
    ...
    for (let i = 0; i < history.length - 1; i++) {
      ...
      let col = this.hexToP5Color(
        DriveVisualization.getDriveStateColor(history[i].driveState)
      );
      stroke(col);
    ...
    if (history.length > 0) {
      ...
      const recentCol = this.hexToP5Color(
        DriveVisualization.getDriveStateColor(
          history[history.length - 1].driveState
        )
      );
  ...
  renderCurrentDriveLines() {
    ...
    // get the O color
    stroke(
      DriveVisualization.getDriveStateColor(DriveState.O_SATISFACTION_SEARCH)
    );
    line(currentO, 0, currentO, this.size);

    // get the P color
    stroke(
      DriveVisualization.getDriveStateColor(DriveState.P_SATISFACTION_SEARCH)
    );
    line(0, currentP, this.size, currentP);
    ...
    text(
      "P: " + this.driveManager.getDriveValues().P_drive.toFixed(0),
      2,
      -(currentP + 7)
    );
    ...
    rotate(-PI / 2);
    text(
      "O: " + this.driveManager.getDriveValues().O_drive.toFixed(0),
      2,
      currentO - 7
    );

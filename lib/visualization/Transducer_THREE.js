...
export class Transducer_THREE extends THREE.Object3D {
  ...
  constructor(
    ...
    if (!Object.values(Transducer_THREE.TRANSDUCER_TYPE).includes(transducerType)) {
      throw new Error(
  ...
  #createFieldHelper() {
    ...
    for (let i = 1; i < pointsXZ.length; i++) {
      const geometry = new THREE.BufferGeometry().setFromPoints([pointsXZ[0], pointsXZ[i]]);
    ...
    for (let i = 1; i < pointsYZ.length; i++) {
      const geometry = new THREE.BufferGeometry().setFromPoints([pointsYZ[0], pointsYZ[i]]);
      const material = new THREE.LineBasicMaterial({ color });
      const line = new THREE.Line(geometry, material);
      this.#fieldOfEffect_HelperGroup.add(line);
    }
    ...
    const arcGeometryXZ = new THREE.BufferGeometry().setFromPoints(arcXZ.getPoints(50));
    ...
    const arcGeometryYZ = new THREE.BufferGeometry().setFromPoints(arcYZ.getPoints(50));
  ...
  updateDirectionHelper() {
    if (this.#directionHelper) {
      this.#directionHelper.setDirection(this.getWorldDirection(new THREE.Vector3()));
    }
  }
  ...
  set fieldOfEffect_HelperRange(value) {
    if (typeof value !== 'number' || value <= 0) {
      throw new Error('Field of effect helper range must be a positive number.');
    }

// Import the Three.js library (no DebugUtility import needed anymore)
...
export class MotionProfile {
  ...
  generateMotionProfile() {
    ...
    if (isNaN(accelerationTime) || isNaN(accelerationDistance)) {
      throw new Error('Failed to calculate acceleration time or distance. Check input values.');
    }
    ...
    if (cruisingDistance < 0) {
      throw new Error(
        'Invalid cruising distance calculated. Check input values.' +
          ` cruisingDistance=${cruisingDistance}`
      );
    }
    ...
    for (let t = 0; t <= accelerationTime; t += this.timestep) {
      ...
      if (isNaN(currentPosition) || isNaN(currentVelocity)) {
        throw new Error('Failed to calculate position or velocity during acceleration phase.');
      }
    ...
    for (let t = 0; t <= accelerationTime; t += this.timestep) {
      ...
      if (isNaN(currentPosition) || isNaN(currentVelocity)) {
        throw new Error('Failed to calculate position or velocity during deceleration phase.');
      }
  ...
  updateMinMax(position, velocity, acceleration) {
    ...
    this._profile_minAcceleration = Math.min(this._profile_minAcceleration, acceleration);
    this._profile_maxAcceleration = Math.max(this._profile_maxAcceleration, acceleration);
  }
  ...
  toString() {
    return `Motion Profile Summary:
      ...
      MP Jerk: min=${(-this.maxJerk).toFixed(2)}, max=${this.maxJerk.toFixed(2)} (Placeholder)`;
  }

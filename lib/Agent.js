...
export class Agent {
  constructor(name, frameRate, x = 0, y = 0, z = 0, initialRotationX = 0) {
    ...
    this.orientation = createVector(1, 0, 0).rotate(
      this.initialRotationRadians
    ); // Apply initial rotation

    // Initialize the Horizontal Control Subsystem (Oscillator)
    this.HorizontalControlSubsystem = new Oscillator(
      -60,
      60,
      0,
      1.0,
      frameRate
    ); // Define min, max, and reinforcement positions
  ...
  updateOrientationBasedOnSubsystem() {
    ...
    const additionalAngleInDegrees =
      this.HorizontalControlSubsystem.sensePosition();
    ...
    const combinedAngle =
      this.initialRotationRadians + additionalAngleInRadians;

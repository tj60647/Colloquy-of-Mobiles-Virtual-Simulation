...
export class DriveManager {
  ...
  constructor(
    ...
    this.validateDriveParameters(O_initialValue, O_floor, O_lowerLimit, O_upperLimit, O_max, 'O');
    this.validateDriveParameters(P_initialValue, P_floor, P_lowerLimit, P_upperLimit, P_max, 'P');
    this.validateIncrementDecrement(increment, 'increment');
    this.validateIncrementDecrement(decrement, 'decrement');

    // Initialize drives
    this.O_drive = new Drive(O_initialValue, O_floor, O_lowerLimit, O_upperLimit, O_max);
    this.P_drive = new Drive(P_initialValue, P_floor, P_lowerLimit, P_upperLimit, P_max);
  ...
  getDriveState() {
    ...
    if (O_drive < this.O_drive.lowerLimit && P_drive < this.P_drive.lowerLimit) {
      return DriveState.SATISFIED_AND_INDIFFERENT;
    } else if (O_drive > this.O_drive.upperLimit && P_drive > this.P_drive.upperLimit) {
   ...
   */
  validateDriveParameters(initialValue, floor, lowerLimit, upperLimit, max, driveName) {
    if (
    ...
    if (initialValue < floor || initialValue > max) {
      throw new Error(`${driveName} drive initial value must be between floor and max.`);
    }
    if (floor < 0) {
      throw new Error(`${driveName} drive floor must be greater than or equal to 0.`);
    }

...
describe('SceneGraphLoader v2', () => {
  it('should load config_v2.json and create all 6 Mobiles', async () => {
    ...
    expect(mobileNames).toEqual(['Beam', 'Female 1', 'Female 2', 'Female 3', 'Male 1', 'Male 2']);
    ...
    const malesAndBeam = environment.mobiles.filter(
      (m) => m.name.startsWith('Male') || m.name === 'Beam'
    );

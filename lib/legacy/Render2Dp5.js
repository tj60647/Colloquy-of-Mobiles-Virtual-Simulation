...
export class Render2Dp5 {
  ...
  render() {
    this.environment.agents.forEach((agent) => {
      ...
      if (this.showOrientation) {
        ...
        textAlign(CENTER);
        text(
          `Orientation: ${agent.HorizontalControlSubsystem.sensePosition().toFixed(2)}`,
          posX,
          posY - diameter / 1.5
        );
      }
      ...
      if (!currentProfile || currentProfile.profile.length === 0) {
        ...
        const currentProfileIndex = agent.HorizontalControlSubsystem.currentProfileIndex;
        const currentProfilePoint = currentProfile.profile[currentProfileIndex] || {};

        const currentAcceleration = currentProfilePoint.acceleration?.toFixed(2) || 'N/A';
        const currentVelocity = currentProfilePoint.velocity?.toFixed(2) || 'N/A';
        ...
        if (this.showTargetPosition) {
          text(
            `Target: ${agent.HorizontalControlSubsystem.targetPosition?.toFixed(2) || 'N/A'}`,
            posX - 10,
            posY + 60
          );
        }
  ...
  drawMotionProfile(motionProfile) {
    ...
    for (let i = 0; i < motionProfile.profile.length; i += 10) {
      ...
      let y = map(motionProfile.profile[i].position, minPosition, maxPosition, height, 0); // Using position as y-coordinate
      vertex(x, y);
    }
    endShape();
    ...
    noStroke();
    textAlign(RIGHT);
    text(`Profile Length: ${motionProfile.profile.length}`, width - 10, height - 10);
  }

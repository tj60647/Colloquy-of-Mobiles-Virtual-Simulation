...
export function createCameraControl(
  ...
  if (!perspectiveCamera) {
    perspectiveCamera = createPerspectiveCamera(perspectiveCameraDefaultPosition);
  }

  // If orthographicCamera is not passed, create a new one
  if (!orthographicCamera) {
    orthographicCamera = createOrthographicCamera(orthographicCameraDefaultPosition);
  }
  ...
  if (!orthographicOrbitControl) {
    ...
    orthographicOrbitControl = createOrbitControls(orthographicCamera, renderer, {
      enableDamping: false,
  ...
  toggleCameraButton.addEventListener('click', () => {
    cameraControlContainer.isPerspective = !cameraControlContainer.isPerspective;
    updateCameraType();
  });
  ...
  resetCameraButton.addEventListener('click', () => {
    if (cameraControlContainer.isPerspective) {
      cameraControlContainer.perspectiveCamera.position.copy(
        cameraControlContainer.perspectiveCameraDefaultPosition
      );
    } else {
      cameraControlContainer.orthographicCamera.position.copy(
        cameraControlContainer.orthographicCameraDefaultPosition
      );
  ...
  orbitModeButton.addEventListener('click', () => {
    ...
    if (cameraControlContainer.isOrbiting) {
      ...
      orbitRadius = Math.sqrt(
        cameraControlContainer.camera.position.z * cameraControlContainer.camera.position.z +
          cameraControlContainer.camera.position.x * cameraControlContainer.camera.position.x
      );
  ...
  function updateCameraType() {
    if (cameraControlContainer.isPerspective) {
      ...
      cameraControlContainer.currentOrbitControl = cameraControlContainer.perspectiveOrbitControl;
      cameraControlContainer.currentOrbitControl.enabled = !cameraControlContainer.isOrbiting;
      ...
      cameraControlContainer.currentOrbitControl = cameraControlContainer.orthographicOrbitControl;

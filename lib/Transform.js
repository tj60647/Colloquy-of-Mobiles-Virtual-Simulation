...
class Transform {
   ...
   */
  constructor(parent, localPosition, localOrientation, name = "Unnamed Transform", id = Date.now()) {
    ...
    this.lastParentGlobalPosition = this.parent ? this.parent.getGlobalPosition() : null;
    this.lastParentGlobalOrientation = this.parent ? this.parent.getGlobalOrientation() : null;
  }
  ...
  applyRotation(vector, yaw, pitch, roll) {
    ...
    rotatedVector = createVector(
      rotatedVector.x * cos(radians(roll)) - rotatedVector.y * sin(radians(roll)),
      rotatedVector.x * sin(radians(roll)) + rotatedVector.y * cos(radians(roll)),
      rotatedVector.z
    );
    ...
    rotatedVector = createVector(
      rotatedVector.x,
      rotatedVector.y * cos(radians(pitch)) - rotatedVector.z * sin(radians(pitch)),
      rotatedVector.y * sin(radians(pitch)) + rotatedVector.z * cos(radians(pitch))
    );
   ...
   */
  parentHasChanged() {
    const currentParentPosition = this.parent ? this.parent.getGlobalPosition() : null;
    const currentParentOrientation = this.parent ? this.parent.getGlobalOrientation() : null;

    return JSON.stringify(this.lastParentGlobalPosition) !== JSON.stringify(currentParentPosition) ||
           JSON.stringify(this.lastParentGlobalOrientation) !== JSON.stringify(currentParentOrientation);
  }
